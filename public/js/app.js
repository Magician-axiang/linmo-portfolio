...
window.App = (function () {
  ...
  function renderContact(contact) {
    ...
    const socialIcons = (contact.socials || [])
      .map(
        (s) =>
          `<a href="${esc(s.url)}" class="social" aria-label="${esc(s.name || "社交链接")}" title="${esc(s.name || "")}" target="_blank" rel="noopener">${socialIcon(s.name)}</a>`
      )
      .join("");
    ...
    el.innerHTML = `
      <div class="lg:col-span-5 reveal">
        ...
        <div class="mt-8 flex flex-wrap gap-3">${socialIcons}</div>
      </div>
      <div class="lg:col-span-7 reveal">
  ...
  function socialIcon(name) {
    ...
    if (n === "kuaishou" || n === "快手") return '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8.5 4c-3 0-5.5 2.2-5.5 5s2.5 5 5.5 5c.6 0 1.2-.1 1.7-.3L13 17l-.7-2.2c1.7-.8 3-2.4 3-4.3 0-2.8-2.5-5-5.5-5zm.5 2.5c.6 0 1 .4 1 1s-.4 1-1 1-1-.4-1-1 .4-1 1-1zm-2 4c-.6 0-1-.4-1-1s.4-1 1-1 1 .4 1 1-.4 1-1 1z"/><path d="M16 10c2.2 0 4 1.8 4 4s-1.8 4-4 4c-.5 0-1-.1-1.4-.2L13 19l.5-1.8c-1-.5-1.8-1.5-1.8-2.7 0-2.2 2-4 4.3-4zm-1 2.5c-.3 0-.5.2-.5.5s.2.5.5.5.5-.2.5-.5-.2-.5-.5-.5zm2 2c-.3 0-.5.2-.5.5s.2.5.5.5.5-.2.5-.5-.2-.5-.5-.5z"/></svg>';
